...
function addPhoto(userId, url, title, description) {
    return knex('photos')
        ...
        .then(photoId => {
            var query = `select p.photo_id as id, url as imageUrl, title, description, p.user_id as author_id, u.name as user, created as uploadDate, updated as editDate, avg(ratingTable.rating) as commonRating, commentsTable.comments as commentsLength, userRatingTable.rating as userRating
        from photos p
        left join (select user_id, name from users) u
        on p.user_id = u.user_id
        left join (select photo_id, rating from ratings) as ratingTable
